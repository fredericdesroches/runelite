...
public class ZulrahCurrentPhaseOverlay extends Overlay {
    ...
    @Inject
    public ZulrahCurrentPhaseOverlay(final ZulrahPlugin plugin, final ZulrahOverlayUtility zulrahOverlayUtility) {
        this.plugin = plugin;
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        final int currentPhase = instance.getCurrentPhaseCounter();
        final ZulrahPhase phase = pattern.getPhase(currentPhase);

        this.panelComponent.getChildren().clear();
        this.panelComponent.getChildren().add(
            TitleComponent.builder().text("Current" + (phase.isJad() ? " : JAD" : "")).color(Color.RED)
